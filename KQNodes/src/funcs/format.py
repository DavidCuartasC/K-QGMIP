...
def fmt_k_particion_q(
    ...
    """
    Formatea una k-partición (k >= 2) reutilizando `fmt_parte_q`.

    Generaliza `fmt_biparticion_q` a k partes: cada grupo es una lista de nodos
    `(tiempo, idx)` (tiempo = EFFECT -> purview en mayúsculas; ACTUAL -> mecanismo
    en minúsculas). Se concatenan en una línea las tapas (numeradores) de las k
    partes y debajo sus bases (denominadores).

    Args:
        grupos: lista de k grupos, cada uno con tuplas (tiempo, idx).
        to_sort: si se ordena cada parte por índice de nodo.

    Returns:
        str: representación multilínea de la k-partición.
    """
    tapas, bases = [], []
    for grupo in grupos:
        tapa, base = fmt_parte_q(list(grupo), to_sort)
        tapas.append(tapa)
        bases.append(base)
    return f"{''.join(tapas)}\n{''.join(bases)}\n"
