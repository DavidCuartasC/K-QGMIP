...
Esto evita los dos muros de memoria de N grande en KQNodes:
  - No se carga la TPM completa en float64.
  - No se construyen los N n-cubos a la vez en ``System.__init__``
    (``src/models/core/system.py`` reshape ``(2,)*N`` por nodo).
El pico de RAM es ~una columna (2^25 int8 = 33.5 MB) más los intermedios de la
marginalización.

El layout ``(N, 2^N)`` con fila = columna del nodo coincide exactamente con la
construcción LIL_ENDIAN de ``System`` (``tpm[:, idx].reshape((2,)*N)``), que es la
notación por defecto del proyecto (``Application.indexado_llegada``).
"""
