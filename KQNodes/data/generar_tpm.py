"""Genera/convierte TPMs al formato columnar binario (N, 2^N) para N grande (KQNodes).
...
El layout (N, 2^N) con fila = columna del nodo coincide con la construcción LIL_ENDIAN
de ``System`` (``tpm[:, idx]``), notación por defecto del proyecto, de modo que
``ColumnStore.columna_ncubo(i)`` reproduce exactamente el n-cubo i del sistema exacto.

NOTA: las redes generadas con ``generar`` son **sintéticas** (sin semántica); sirven
para medir escalabilidad, igual que en KGeoMIP. El artefacto N=25 de KGeoMIP
(``KGeoMIP/data/samples/N25A_cols.npy``) tiene el mismo formato y puede reutilizarse.

Uso (desde KQNodes/):
    uv run python data/generar_tpm.py generar --n 25 --seed 0
    uv run python data/generar_tpm.py desde-csv --csv src/.samples/N10A.csv
"""
...
KQNODES_ROOT = Path(__file__).resolve().parents[1]  # .../KQNodes (este archivo: KQNodes/data/)
SAMPLES = KQNODES_ROOT / "data" / "samples"
...
def desde_csv(csv_path: Path) -> Path:
    ...
    np.save(salida, cols)
    print(
        f"Guardado: {salida}  (N={n}, {dtype.__name__}, binaria={binaria}, "
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Genera/convierte TPMs columnar (KQNodes).")
