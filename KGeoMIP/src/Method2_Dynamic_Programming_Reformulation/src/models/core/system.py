...
class System:
    ...
    def k_partir(
        ...
        """
        Genera el sistema k-particionado.

        grupos[i] = (alcance_i, mecanismo_i)
          alcance_i:   indices de n-cubos futuros en el grupo i
          mecanismo_i: indices de dimensiones presentes en el grupo i

        Cada n-cubo cuyo índice esté en alcance_i es marginalizado sobre todas
        sus dims excepto mecanismo_i, forzando independencia entre grupos.
        """
        new_sys = System.__new__(System)
        new_sys.estado_inicial = self.estado_inicial
        ncubos_resultado = []
        for cube in self.ncubos:
            i = 0
            while i < len(grupos) and cube.indice not in grupos[i][0]:
                i += 1
            if i < len(grupos):
                alc_i, mec_i = grupos[i]
                ncubos_resultado.append(
                    cube.marginalizar(np.setdiff1d(cube.dims, mec_i))
                )
        new_sys.ncubos = tuple(ncubos_resultado)
        return new_sys
