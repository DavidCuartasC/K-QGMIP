...
def ejecutar_kgeomip(
    ...
    *,
    ...
    """Ejecuta KGeoMIP sobre un subsistema y muestra la k-MIP encontrada.

    Args:
        estado_inicio: estado inicial binario (define N y la TPM NxA a cargar).
        ...
        condiciones: condiciones de fondo; por defecto todas activas.
        mode: 'auto' (def.), 'exact', 'bnb' o 'heuristic'.
        timeout: segundos máximos para el B&B (None = sin límite).

    Returns:
        Solution: solución con la pérdida δₖ y la k-partición formateada.
    """
    condiciones = condiciones or ("1" * len(estado_inicio))
    tpm_path = resolver_tpm_path(estado_inicio)
    tpm = np.genfromtxt(tpm_path, delimiter=",")

    gestor = Manager(estado_inicial=estado_inicio)
    estrategia = KGeoMIP(gestor)
    solucion = estrategia.aplicar_estrategia_k(
        condiciones, alcance, mecanismo, tpm, k, mode=mode, timeout=timeout
    )
    print(solucion)
    return solucion
...
def iniciar_kgeomip():
    """Demo de KGeoMIP. Configurable por entorno: KGEOMIP_ESTADO, KGEOMIP_ALCANCE,
    KGEOMIP_MECANISMO, KGEOMIP_K (k por defecto = 3)."""
    estado_inicio = os.getenv("KGEOMIP_ESTADO", "100")
    n = len(estado_inicio)
    alcance = os.getenv("KGEOMIP_ALCANCE", "1" * n)
    mecanismo = os.getenv("KGEOMIP_MECANISMO", "1" * n)
    k = int(os.getenv("KGEOMIP_K", "3"))
    return ejecutar_kgeomip(estado_inicio, alcance, mecanismo, k)
